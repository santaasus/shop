...
func GetUserByParams(params map[string]any) (*domain.User, error) {
	user, err := modifyOrGetUserBy(SQLSELECT, params)

	return user, err
}

func UpdateUserByParams(params map[string]any) (*domain.User, error) {
	user, err := modifyOrGetUserBy(SQLUPDATE, params)

	return user, err
}

func modifyOrGetUserBy(sqlAction string, params map[string]any) (*domain.User, error) {
	db, err := dbCore.Connect()
	if err != nil {
		return nil, err
	}
